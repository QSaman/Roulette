...
\begin{document}
	...
	\par As an example, let's assume we have 3 chips with the value of \$$\frac{1}{3}$. We bet one of them on black, the other on an even number and the other on a number greater than 18. Note that all of our bets are in the same round. The expected value is:
	\begin{equation*}
		\begin{split}
			E &= \frac{3}{3} \times p^3 \\
			  &+ (\frac{2}{3} - \frac{1}{3}) \times p^2 \times (1 - p) \\
			  &+ (\frac{1}{3} - \frac{2}{3}) \times p \times (1 - p)^2 \\
			  &+ (-\frac{3}{3}) \times (1 - p)^3 \\
			  &= -0.023
		\end{split}
	\end{equation*}
